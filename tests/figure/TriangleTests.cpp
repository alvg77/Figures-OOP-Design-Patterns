...
TEST_CASE("Triangle with small decimal sides calculates correctly", "[triangle][perimeter]") {
    const Triangle triangle(0.1, 0.2, 0.25);
    REQUIRE_THAT(triangle.perimeter(), Catch::Matchers::WithinRel(0.55, 1e-10));
}
...
TEST_CASE("Triangle construction fails with all negative sides", "[triangle][invalid]") {
    REQUIRE_THROWS_AS(Triangle(-1, -2, -3), std::invalid_argument);
}
...
TEST_CASE("Triangle construction fails with all zero sides", "[triangle][invalid]") {
...
TEST_CASE("Triangle fails with negative infinity", "[triangle][special]") {
    REQUIRE_THROWS_AS(Triangle(-std::numeric_limits<double>::infinity(), 1, 1),
                      std::invalid_argument);
}
...
TEST_CASE("Triangle fails with sides causing perimeter overflow", "[triangle][overflow]") {
    constexpr double val = 1.7976931348623157E+308;
    REQUIRE_THROWS_AS(Triangle(val, val, val), std::invalid_argument);
}
...
TEST_CASE("Triangle with mixed magnitude sides", "[triangle][edge]") {
    const Triangle triangle(1e-5, 1e-5, 1e-5);
    REQUIRE_THAT(triangle.perimeter(), Catch::Matchers::WithinRel(3e-5, 1e-10));
}
